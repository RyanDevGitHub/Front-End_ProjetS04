...
let emit = defineEmits(['CreateVol']);
let numerosvol = ref("");
let villeDepart = ref("");
let villeArriv = ref("");
let date_depart = ref("");
...
let idAvion = ref("");
let nbPlace = ref("");
async function CreateVol(villeDepart: string, villeArriv: string, date_depart: string, date_arrivee: string, heure_depart: string, heure_arrivee: string, idAvion: string, nbPlace: string) {
    //crée un vol avec la methode fetch 
    const url = 'https://localhost:7232/api/Vols/';
    const data = { villeDepart: villeDepart, villeArrivee: villeArriv, DateDepart: date_depart, DateArriver: date_arrivee, heureDepart: heure_depart, heureArrivee: heure_arrivee, idAvion: idAvion, nombrePlaces: nbPlace };
    const options = {
        method: 'POST',
...
<template>
    <form
        @submit.prevent="CreateVol(villeDepart, villeArriv, date_depart, date_arrivee, heure_depart, heure_arrivee, idAvion, nbPlace)">
        <div class="form-row">
            <div class="form-column">
                ...
                <input type="date" id="DateDepart" name="DateDepart" required v-model="date_depart">
        ...
        <button type="submit" class="ajouterVol-button">Ajouter
            un vol</button>
    </form>
</template>
